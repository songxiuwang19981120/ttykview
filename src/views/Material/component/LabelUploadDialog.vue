	...
	<el-dialog title="标签上传" :visible="showDialog" @close="btnCancel">
		...
		<el-form :model="ruleForm" ref="ruleForm" :rules="rules" label-width="120px">
			<el-form-item label="账号分组选择：" prop="grouping_id">
				<el-select v-model="ruleForm.grouping_id" placeholder="账号分组选择" style="margin-right: 20px" clearable @change="searchEquipmentChange" size="medium">
			...
			<el-form-item prop="label" label="添加标签：">
				<el-input type="textarea" placeholder="请输入标签(一行仅限一个)" rows="6" v-model="ruleForm.label" size="medium"
					style="width: 60%">
				</el-input>
			</el-form-item>
		</el-form>
		...
		async btnOK() {
			try {
				...
				if (nickNameArr.length && nickNameArr[0]) {
					...
					return this.$message.warning('标签内容不能为空')
				}
