		...
		>
			<el-form ref="videoForm" :rules="rulesUpload" :model="videoForm" label-width="140px">
				...
				<el-form-item label="分类:" prop="library">
					<TypeSelect
						ref="addtypeselect"
						@handleTypeChange="handleAddTypeChange($event)"
		...
		<div>
			<el-checkbox
				...
				<span style="color: #ff411f; font-size: 12px; padding-left: 20px">
					已选中 {{ checkedCities.length }} 个视频</span
				></el-checkbox
			>
			<el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange" class="video">
				<div v-for="(item, index) in tableData" :key="index" class="videoData">
					...
					<el-checkbox :label="item.material_id" :key="item.material_id" class="videoNum"
						>视频编号:{{ item.video_num }}</el-checkbox
					>
